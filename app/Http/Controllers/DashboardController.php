    ...
    public function index()
    {
        ...
        $contractsFinishing = Contract::where(function ($query) {
            ...
            ->select(
                "*",
                DB::raw(
                    "IF(DATE_ADD(effective_at, INTERVAL 11 MONTH) <= CURRENT_DATE() AND DATE_ADD(effective_at, INTERVAL 1 YEAR) >= CURRENT_DATE(), 1, 0) AS status"
                )
            )
            ->get();
        ...
        $alta = Contract::where("status_id", 1)->count();
