<x-app-layout>
    <x-slot name="header" class="pt-8">
        <div class="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-300 flex-wrap">
            ...
            <span>/</span>
            <span class="text-blue-600 dark:text-blue-300 font-semibold">{{ __('Campanhas') }}</span>
        </div>
        <div class="mt-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    Gere as campanhas e associa-as rapidamente às empresas fornecedoras.
                </p>
            </div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <section class="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
                <header class="border-b border-slate-100 px-6 py-4 dark:border-gray-700">
                    <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100">{{ __('Lista de Campanhas') }}</h2>
                </header>
                    ...
                    <table class="min-w-full divide-y divide-gray-200 text-sm text-gray-700 dark:divide-gray-700 dark:text-gray-200">
                        <thead class="bg-gray-50 dark:bg-gray-900">
                            <tr>
                                ...
                                <th scope="col" class="py-3 px-4 text-left font-semibold uppercase tracking-wide">{{ __('Empresa Associada') }}</th>
                                <th scope="col" class="py-3 px-4 text-left font-semibold uppercase tracking-wide">{{ __('Nome da Campanha') }}</th>
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                            @forelse ($plans as $plan)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-900/40 transition">
                                    <td class="py-3 px-4 font-semibold text-gray-900 dark:text-gray-100">{{ $plan->id }}</td>
                                    <td class="py-3 px-4">
                                        {{ $plan->provider?->title ?? '—' }}
                                    </td>
                                    <td class="py-3 px-4">{{ $plan->title }}</td>
