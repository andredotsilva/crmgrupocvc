<x-app-layout>
    <x-slot name="header" class="pt-8">
        ...
        <div class="flex items-end justify-between">
            <h4 class="mr-4 text-xl	dark:text-gray-200">{{ __(' Editar Provedor - ') }}{{ $providers->acronym }}</h4>
            <a href="{{ url()->previous() }}">
                <button
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden  sm:rounded-lg">
                <div class="gap-x-6 gap-y-8 sm:grid-cols-6 px-6 pb-2 pt-4 rounded-2xl bg-white dark:bg-gray-800">
                    <div class="max-w-md mx-auto bg-white rounded-md p-6 dark:bg-gray-800">
                        <h2 class="text-lg font-semibold mb-4 dark:text-gray-200">Editar Provedor</h2>
                        <form action="{{ route('providers.update', $providers->id) }}" method="POST">
                            @csrf
                            @method('PUT')
                            <div class="mb-4">
                                <label class="block text-gray-700 font-bold mb-2 dark:text-gray-200"
                                    for="name">Acronimo:</label>
                                <input type="text" id="name" name="acronym" value="{{ $providers->acronym }}"
                                    class="border border-gray-300 rounded-md px-3 py-2 w-full">
                            </div>
                            <div class="mb-4">
                                <label class="block text-gray-700 font-bold mb-2 dark:text-gray-200"
                                    for="text">Titulo:</label>
                                <input type="text" id="text" name="title" value="{{ $providers->title }}"
                                    class="border border-gray-300 rounded-md px-3 py-2 w-full">
                            </div>
                            <button type="submit"
