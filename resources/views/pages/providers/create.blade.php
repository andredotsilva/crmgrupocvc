...
<x-app-layout>
    <x-slot name="header" class="pt-8">
        ...
        <div class="flex items-end justify-between">
            <h4 class="mr-4 text-xl	dark:text-gray-200">{{ __(' Inserir Provedor ') }}</h4>
            <a href="{{ url()->previous() }}">
                <button
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden  sm:rounded-lg">
                <div class="gap-x-6 gap-y-8 sm:grid-cols-6 px-6 pb-2 pt-4 rounded-2xl bg-white dark:bg-gray-800">
                    <div class="max-w-md mx-auto bg-white rounded-md p-6 dark:bg-gray-800">
                        <h2 class="text-lg font-semibold mb-4 dark:text-gray-200">Inserir Provedor</h2>
                        @if (session('success'))
                        ...
                        <form method="POST" action="{{ route('providers.store') }}">
                            <div class="mb-4">
                                ...
                                <input type="text" id="name" name="acronym" required
